...
public static class Triangulator
{
    ...
    public struct Triangle
    {
        ...
        public Triangle(Vector2 _a, Vector2 _b, Vector2 _c, int _d,int _e, int _f)
        {
            a = _a;
            b = _b;
            c = _c;
     ...
     */
    public static void RotatePathClockwise(ref List<Vector2> path, ref List<int> id)
    {
     ...
     */
    public static List<Triangle> Triangulate(List<Vector2> pathSource, List<int> id, int subsequentCalls = 0)
    {
        ...
        if (!segmentValid)
        {
            ...
            RotatePathClockwise(ref path, ref id);
            return Triangulate(path, id, subsequentCalls + 1);
        }

        //Ok! the segment being valid, let's extract that triangle from the path and carry on.
        Triangle firstTri = new Triangle(path[0], path[1], path[path.Count - 1], id[0], id[1], id[path.Count - 1]);
        path.RemoveAt(0);
        id.RemoveAt(0);
        if (path.Count < 3)
        {
        ...
        else
        {
            ...
            result.AddRange(Triangulate(path, id));
            return result;
        }
    }
}
